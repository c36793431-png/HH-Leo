...
import { FEED_REQUEST_STATUSES, type FeedRequestStatus } from "@/lib/feed-requests";
import type { ActionResult } from "@/lib/action-result";
import { emitToast } from "@/lib/toast-bus";

type Action = (prevState: ActionResult | null, formData: FormData) => Promise<ActionResult>;

interface FeedRequestRowActionsProps {
 ...
 * /admin/feed-requests row — mirrors ApplicationRowActions. */
export function FeedRequestRowActions({
  ...
  function handleStatusChange(next: string) {
    ...
    formData.append("id", feedRequestId);
